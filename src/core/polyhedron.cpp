...
void Polyhedron::removeFromFront(front::Edge* fEdge) {
    ...
    auto rem_iter = std::find(m_frontEdges.begin(), m_frontEdges.end(), fEdge);
    *rem_iter = m_frontEdges.back();
    m_frontEdges.pop_back();
    delete fEdge;
}
...
bool Polyhedron::segmentIntersectMesh(const vec3& v0, const vec3& v1) const {
    for (auto& face : m_innerFaces) {
        if (spt::algs::doesSegmentIntersectTriangle(
            v0, v1,
...
bool Polyhedron::potentialEdgeIntersectFront(front::Edge* fEdge) const {
    ...
    for (auto& fedge : m_frontEdges) {
        ...
        if (contains[0]) {
            ...
            if (spt::algs::distancePointToSegment(vert_buf->pos(), std::get<0>(opp_verts)->pos(), std::get<1>(opp_verts)->pos()) < C_EDGES_INTERS_DIST * m_prefLen)
                return true;
        } else if (contains[1]) {
...
bool Polyhedron::anyVertInsidePotentialTetrCheck(front::Edge* fEdge) const {
    ...
    for (auto& vert : m_innerVerts) {
        if (vert != std::get<0>(opp_verts) &&
            ...
            spt::algs::isPointInTetrahedron(vert->pos(), points[0], points[1], points[2], points[3]))
            return true;
    }
...
bool Polyhedron::doesFrontIntersectSphere(const vec3& center, real_t radius) const {
    for (auto& fface : m_frontFaces) {
        ...
        if (spt::algs::doesTriangleIntersectSphere(triangle[0], triangle[1], triangle[2], center, radius))
            return true;
    }
...
void Polyhedron::setFEdgesInFrontSplit(const front::Edge* fEdge, std::array<front::Edge*, 2> newOppFEdges, std::array<front::Face*, 2> newFFaces, pair_ff oppFFaces) const {
    ...
    vec3 main_vert1_pos = newFFaces[1]->face->contains(fEdge->edge->verts[0]) ?
        fEdge->edge->verts[0]->pos() : fEdge->edge->verts[1]->pos();
    vec3 main_vert0_proj = spt::algs::project(main_vert0_pos, opp_verts_poses[0], opp_verts_poses[1]);
    vec3 main_vert1_proj = spt::algs::project(main_vert1_pos, opp_verts_poses[0], opp_verts_poses[1]);
    ...
    vec3 adj_opp_proj0 = spt::algs::project(adj_opp_pos0, opp_verts_poses[0], opp_verts_poses[1]);
    vec3 adj_opp_proj1 = spt::algs::project(adj_opp_pos1, opp_verts_poses[0], opp_verts_poses[1]);
...
bool Polyhedron::tryComputeNewVertPosType3(front::Face* fFace, vec3& out_pos) {
    ...
    vec3 new_pos = spt::algs::lineIntersectPlane(v2_pos, e, v0_pos, v1_pos, v0_pos + e_mn2);
...
pair_rr Polyhedron::analyzeMeshQuality(std::vector<Tetr*>::iterator* out_minQualityTetr) {
    std::size_t n_tetrs = 0;
    real_t av_q = static_cast<real_t>(0);
    real_t min_q = static_cast<real_t>(1);
    auto min_q_tetr = m_innerTetrs.begin();
...
const std::vector<front::Face*>& Polyhedron::frontFaces() const {
    return m_frontFaces;
}


const std::vector<front::Edge*>& Polyhedron::frontEdges() const {
    return m_frontEdges;
}
