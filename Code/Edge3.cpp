    ...
    vector<unique_ptr<Facet3>*> &facets)
{
    ...
    find2AdjFacets(facets, around_facets[0], around_facets[1]);
...
void Edge3::findAdjFacets(const vector<unique_ptr<Facet3>*>& facets, vector<unique_ptr<Facet3>*>& adjFacets)
{
    ...
    vector<unique_ptr<Vertex3>*> &verts)//, vector<ShellEdge3*>& shellEdges, vector<ShellFacet3*>& shellFacets)
{
    ...
    vector<unique_ptr<Facet3>*> old_facets;
    findAdjFacets(facets, old_facets);
...
double FrontEdge3::getAngleExCos(const Crystallite3* crys)
{
    if (!needProcessing)
        return _exCos;
    
    return computeAngleExCos(crys);
}

double FrontEdge3::computeAngleExCos(const Crystallite3* crys)
{
    unique_ptr<FrontFacet3>* f_facets[2];
    crys->findAdjacentFrontFacets(getPtrToUPtr(), f_facets[0], f_facets[1]);
...
double FrontEdge3::computeAngleExCos_OLD(const Crystallite3* crys)
{
    unique_ptr<FrontFacet3>* f_facets[2];
    crys->findAdjacentFrontFacets(getPtrToUPtr(), f_facets[0], f_facets[1]);
    ...
    facets_inner_vecs[0] = opp_verts_poses[0] - spatialalgs::project(opp_verts_poses[0], (*edge->vertexes[0])->getPosition(), (*edge->vertexes[1])->getPosition());
    facets_inner_vecs[1] = opp_verts_poses[1] - spatialalgs::project(opp_verts_poses[1], (*edge->vertexes[0])->getPosition(), (*edge->vertexes[1])->getPosition());
...
double FrontEdge3::computeAngleCos(bool &out_isConcave, const Crystallite3* crys)
{
    unique_ptr<FrontFacet3>* f_facets[2];
    crys->findAdjacentFrontFacets(getPtrToUPtr(), f_facets[0], f_facets[1]);
    ...
    for (auto &f_facet : crys->frontFacets)
    {
        ...
        if (spatialalgs::isRayIntersectTriangle(
                edge_inner_vec, shell_inside_test,
...
double FrontEdge3::computeAngle(const Crystallite3* crys)
{
    unique_ptr<FrontFacet3>* f_facets[2];
    crys->findAdjacentFrontFacets(getPtrToUPtr(), f_facets[0], f_facets[1]);
...
//void FrontEdge3::findAdjacentFrontFacets(
...
//    }
//
//    throw std::logic_error("Something went wrong in FrontEdge3::findAdjacentFrontFacets");
//}
//
...
{
    // For the future.
    //_adjFacets[0] = nullptr;
    //_adjFacets[1] = nullptr;
}
