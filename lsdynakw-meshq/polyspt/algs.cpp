...
// TODO: remove defines
#define EPS static_cast<vec<3>::real_type>(1e-6)
        ...
        const vec<3>& pl_p0, const vec<3>& pl_p1, const vec<3>& pl_p2)
{
    ...
    vec<3>::real_type det = vec<3>::dot(edges[0], pvec);
    return det > static_cast<vec<3>::real_type>(1e-6) || det < static_cast<vec<3>::real_type>(-1e-6);
}
    ...
    const vec<3>& pl_p0, const vec<3>& pl_p1, const vec<3>& pl_p2)
{
    ...
    vec<3> pvec = vec<3>::cross(dir, edges[1]);
    vec<3>::real_type det = vec<3>::dot(edges[0], pvec);
    if (det < static_cast<vec<3>::real_type>(1e-6) && det > static_cast<vec<3>::real_type>(-1e-6))
        return false;

    vec<3> tvec = origin - pl_p0;
    vec<3> qvec = vec<3>::cross(tvec, edges[0]);

    vec<3>::real_type t = vec<3>::dot(edges[1], qvec) / det;
    out_intersectPoint = origin + dir * t;
    return t > static_cast<vec<3>::real_type>(0.0);
}
    ...
    const vec<3>& tr_p0, const vec<3>& tr_p1, const vec<3>& tr_p2)
{
    ...
    vec<3>::real_type inv_det = static_cast<vec<3>::real_type>(1.0) / det;

    vec<3> tvec = origin - tr_p0;
    vec<3>::real_type u = vec<3>::dot(tvec, pvec) * inv_det;
    if (u < static_cast<vec<3>::real_type>(0.0) || u > static_cast<vec<3>::real_type>(1.0))
        return false;

    vec<3> qvec = vec<3>::cross(tvec, edges[0]);
    vec<3>::real_type v = vec<3>::dot(dir, qvec) * inv_det;
    if (v < static_cast<vec<3>::real_type>(0.0) || u + v > static_cast<vec<3>::real_type>(1.0))
        return false;

    vec<3>::real_type t = vec<3>::dot(edges[1], qvec) * inv_det;
    return t > static_cast<vec<3>::real_type>(0.0);
}
    ...
    const vec<3>& line0_p0, const vec<3>& line0_p1, const vec<3>& line1_p0, const vec<3>& line1_p1)
{
    ...
    vec<3>::real_type a = vec<3>::dot(u, u); // >= 0
    vec<3>::real_type b = vec<3>::dot(u, v);
    vec<3>::real_type c = vec<3>::dot(v, v); // >= 0
    vec<3>::real_type d = vec<3>::dot(u, w);
    vec<3>::real_type e = vec<3>::dot(v, w);
    ...
    return (line0_p0 + u * sc + line1_p0 + v * tc) * static_cast<vec<3>::real_type>(0.5);
}
    ...
    const vec<3>& plane_p0, const vec<3>& plane_p1, const vec<3>& plane_p2)
{
    ...
    vec<3>::real_type t = vec<3>::dot(edges[1], qvec) / vec<3>::dot(edges[0], pvec);
    ...
    const vec<3>& tr_p0, const vec<3>& tr_p1, const vec<3>& tr_p2)
{
    ...
    vec<3>::real_type t = vec<3>::dot(edges[1], qvec) * inv_det;
    return t < static_cast<vec<3>::real_type>(1.0) && t > static_cast<vec<3>::real_type>(0.0);
}
    ...
    const vec<3>& segm1_p0, const vec<3>& segm1_p1)
{
    ...
    sc = std::abs(sn) < static_cast<vec<3>::real_type>(1e-6) ? static_cast<vec<3>::real_type>(0.0) : sn / sd;
    tc = std::abs(tn) < static_cast<vec<3>::real_type>(1e-6) ? static_cast<vec<3>::real_type>(0.0) : tn / td;
    ...
    const vec<3>& start1, const vec<3>& vel1)
{
    ...
    if (dv2 < static_cast<vec<3>::real_type>(1e-6))
        return static_cast<vec<3>::real_type>(0.0);
        ...
        const vec<3>& point, const vec<3>& tetr_p0, const vec<3>& tetr_p1, const vec<3>& tetr_p2, const vec<3>& tetr_p3)
{
    ...
    std::array<vec<3>::real_type, 5> abs_mixed_prods;
