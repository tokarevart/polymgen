...
{ // Pay attention if something wrong.
...
double Vec3::distanceToLine(const Line3& line) const
{
	return (spatialalgs::project(*this, line) - *this).magnitude();
}

double Vec3::distanceToSegment(const Segment3& segment) const
{
	Point3 proj = spatialalgs::project(*this, segment);
	double sqr_magns[2];

	const double EPS = segment.magnitude() * 1e-6;
	if (INSIDE_RECTANGLE(segment.points[0], segment.points[1], proj))
		return (proj - *this).magnitude();
	else if ((sqr_magns[0] = (segment.points[0] - *this).sqrMagnitude()) < (sqr_magns[1] = (segment.points[1] - *this).sqrMagnitude()))
